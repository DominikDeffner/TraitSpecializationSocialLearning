...
# Code for simulations comparing interplay between innovation and specialization for different numbers of learning opportunities (Fig. 3)
    ...
    for (i in 1:Tmax){
      ...
      for (x_ind in Pop$id[which(Pop$Strategy=="Individual")]){
        ...
        if(InnoStyle=="Innovation"){
          ...
          if (SigmaOn == "Yes"){
            Pop$sigma[which(Pop$id==x_ind)] <- runif(1, 0, UpperBoundSigma)
          }
        }
        ...
        if(InnoStyle=="Modification"){
          ...
          if (SigmaOn == "Yes"){
            ...
            while (Pop$sigma[which(Pop$id==x_ind)] < 0 | Pop$sigma[which(Pop$id==x_ind)] > UpperBoundSigma){
              Pop$sigma[which(Pop$id==x_ind)] <- OldSigma + rnorm(1, mean = 0, sd=SigmaModSigma)
            }
          }
        }
        ...
        for (x_soc in sample(Pop$id[which(Pop$Strategy=="Social")])){
          ...
          if (AdaptX > max(Models$Fitness)) {
            ...
            if (SigmaOn == "Yes" & SD_Copy_error != 0){
              ...
              while (Pop$sigma[Pop$id==x_soc] < 0 | Pop$sigma[Pop$id==x_soc] > UpperBoundSigma){
                Pop$sigma[Pop$id==x_soc] <- OldSigma + rnorm(1, mean = 0, sd=0.01)
              }
      ...
      if(i==Tmax){
        Adapt_vector[xsim]   <- mean(Pop$Fitness, na.rm = TRUE)
        Sigma_vector[xsim]   <- mean(Pop$sigma, na.rm = TRUE)
        Xi_vector[xsim]      <- mean(Pop$xi, na.rm = TRUE)
      }
      ...
      if (Innovation == "Yes"){
        ...
        if (SigmaOn == "Yes" & SD_Copy_error !=0){
          ...
          while (Newborn$sigma < 0 | Newborn$sigma > UpperBoundSigma){
            Newborn$sigma <- OldSigma + rnorm(1, mean = 0, sd=0.01)
          }
      ...
      if(ProbEnvChange<e){
        ...
        if(EnvChange=="Gradual0.1"){
          E0 <- E
          E <- E0 + rnorm(1, mean = 0, sd=0.1) 
          while (E < -1 | E > 1) {
            E <- E0 + rnorm(1, mean = 0, sd=0.1) 
          }
        }
...
resultsLearningOpportunities <- mclapply(
  1:nrow(seq) ,
  function(i) simfunct(seq$Nsim[i],seq$N[i], seq$Tmax[i], seq$e[i], seq$Mxi[i], 
                       seq$Mm[i], seq$m[i], seq$SigmaModMu[i], seq$SigmaModSigma[i],seq$Nlearn[i], seq$NumberModels[i], seq$UpperBoundSigma[i],seq$EnvChange[i],seq$SigmaOn[i],seq$SD_Copy_error[i]),
  mc.cores=1)
